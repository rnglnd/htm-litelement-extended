...
`;
	...
	const Section = ({ title, items, addItem }) => html$1`
  <section class=${title.toLowerCase()}>
    <h3 class="main-header">${title}</h3>
    <ul class="main-list">
      ${items.map((item) => html$1`
...
`;
	...
	const html$2 = htm.bind(h);

	const Address = () => html$2`
...
`;
	...
	const html$3 = htm.bind(h);

	const Address$1 = ({ basket, removeItem }) => {
	...
	render(html$4`<${App} />`, document.body);
